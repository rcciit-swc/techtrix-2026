...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (eventsData.length > 0) setRegisteredEvents(eventsData.filter((event) => event.registered));
  }, [eventsData]);
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-cover bg-center bg-fixed" style={{ backgroundImage: "url('/profile/profilebg.jpeg')" }}>
      {/* Dark Overlay */}
      <div className="absolute inset-0 bg-black/70" />

      {/* Animated Background Overlay */}
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(circle at 50% 50%, rgba(250, 204, 21, 0.1) 0%, transparent 70%)', animation: 'pulse 8s infinite' }} />

      <main className="pt-8 pb-10 relative z-10">
        <h1 className="text-center font-bold text-[32px] md:text-[48px] text-white mb-2 tracking-wider uppercase" style={{ fontFamily: "'Metal Mania'" }}>
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          >
            ...
            <motion.div className="flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8" variants={containerVariants} initial="hidden" animate="visible">
              ...
              <motion.div variants={itemVariants} className="relative shrink-0">
                <div className="rounded-full shadow-[0_0_20px_rgba(250,204,21,0.3)]">
                  <Avatar className="w-[160px] h-[160px] border-2 border-yellow-400/30">
                    ...
                    <AvatarImage src={profileImage || '/default-avatar.png'} alt={userData?.name || 'Profile'} onLoad={() => setImageLoaded(true)} className={imageLoaded ? 'block object-cover' : 'hidden'} />
                    <AvatarFallback className="bg-black/60 text-yellow-400 text-4xl" style={{ fontFamily: "'Metal Mania'" }}>{userData?.name?.[0] || 'U'}</AvatarFallback>
                  </Avatar>
                </div>
              </motion.div>
              ...
              <div className="flex flex-col gap-6 items-center md:items-start w-full">
                <motion.div className="text-white text-center md:text-left space-y-1" variants={itemVariants}>
                  <h2 className="font-bold text-[28px] md:text-[36px] uppercase leading-tight text-yellow-400 tracking-wider" style={{ fontFamily: "'Metal Mania'" }}>
                    {userData?.name || name}
                  </h2>
                  <p className="font-medium text-lg text-white/70 font-[Maname]">{userData?.email}</p>
                  <p className="font-medium text-lg text-white/70 font-[Maname]">{userData?.phone}</p>
                </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="mt-8">
            <h2 className="text-center font-bold text-[32px] md:text-[48px] text-white mb-6 relative inline-block w-full uppercase tracking-wider" style={{ fontFamily: "'Metal Mania'" }}>
              Registered Events
            ...
            {registeredEvents.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 justify-items-center">
                {registeredEvents.map((event, index) => (
                  <motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 * index }} className="w-full max-w-[340px]">
                    <EventsCard
                      ...
                      transaction_screenshot={event.transaction_screenshot ?? null}
              ...
              <div className="text-center text-white mt-10">
                <p className="text-lg md:text-xl mb-6 text-white/70 animate-pulse font-[Maname]">You have not registered for any event. Register now!</p>
      ...
      <EditProfileDialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen} userData={userData} profileImage={profileImage} onSave={handleProfileSave} name={name} />
    </div>
  );
}