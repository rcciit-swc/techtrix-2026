...
export default function EventDetails({ event }: Props) {
  ...
  // SWC-paid logic
  const { swcData, communityData, evangelistData } = useUser();
      ...
      case 'info':
        return (
          <div className="space-y-4 sm:space-y-5">
            ...
            {/* Share this Event - Only for Evangelists */}
            {evangelistData && (
              <div id="evangelists" className="animate-fadeIn mt-2 sm:mt-4">
                <h3
                  className="text-sm sm:text-lg text-[#EDF526] mb-3 tracking-[0.1em] flex items-center gap-3 drop-shadow-[0_0_5px_rgba(237,245,38,0.5)]"
                  style={{ fontFamily: "'Metal Mania', cursive" }}
                >
                ...
                <div className="group relative bg-[#0a0a0a]/70 backdrop-blur-md rounded-xl p-4 sm:p-5 border border-yellow-500/20 overflow-hidden transition-all duration-300 hover:border-yellow-400/40">
                  <p className="text-white/60 text-xs sm:text-sm mb-4 font-[Maname]">
                    Share this event using your evangelist referral link!
                  </p>
                  <div className="flex flex-wrap gap-3">
                    <Button
                      onClick={() => {
                        const shareUrl = `${window.location.origin}${window.location.pathname}?ref=${evangelistData.referral_code}`;
                        const plainDesc = (event.description || '')
                          .replace(/<[^>]*>/g, '')
                          .replace(/\s+/g, ' ')
                          .trim()
                          .slice(0, 200);
                        const feeInfo = isSWCFreeCategory
                          ? event.registration_fees > 0
                            ? `*Free for SWC fund holders!*\n_Login with your RCCIIT college domain email and register for FREE if your SWC fund is paid._\nOtherwise, registration fee is *Rs.${event.registration_fees}*.`
                            : `*This event is free for everyone.*`
                          : event.registration_fees > 0
                            ? `*Registration fee: Rs.${event.registration_fees}*`
                            : `*This event is free to enter.*`;
                        const coordinatorLines =
                          event.coordinators && event.coordinators.length > 0
                            ? `*For event queries, contact:*\n${event.coordinators.map((c: { name: string; phone: string }) => `_${c.name.trim()}_ - ${c.phone.trim()}`).join('\n')}`
                            : null;
                        const text = [
                          `*TechTrix 2026 - ${event.name}*`,
                          plainDesc,
                          feeInfo,
                          `*Register now:*\n${shareUrl}`,
                          coordinatorLines,
